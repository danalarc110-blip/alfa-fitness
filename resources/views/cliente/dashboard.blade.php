...
<body class="font-sans antialiased bg-black min-h-screen text-white">
    <div class="max-w-3xl mx-auto py-16 px-6 text-center">
        <h1 class="text-3xl font-bold mb-2">
            ¡Bienvenido, {{ auth('cliente')->user()->nombre }}!
        </h1>
        <p class="text-gray-400 mb-8">Has iniciado sesión correctamente como cliente de Alpha Fitness.</p>
        ...
        <div class="flex items-center justify-center gap-3">
            ...
            <form method="POST" action="{{ route('cliente.logout') }}">
                @csrf
                <button type="submit" title="Cierra tu sesión por completo (también en Google si entraste así)" class="bg-yellow-400 hover:bg-yellow-300 text-black font-semibold px-6 py-3 rounded-xl">
                    Cerrar sesión
                </button>
            </form>
        </div>
